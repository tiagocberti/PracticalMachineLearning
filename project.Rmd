...
First of all we need to extract the column with the outcome `classe` to let the trainning dataset containing only posible predictors.
```{r}
...
The assignment asks to use data from accelerometers on the `belt`, `forearm`, `arm`, and `dumbell`, so the features are extracted based on these keywords.   
...
#zero.var
```
Every feature varies enought to be consideres in the model.
...
Most of training algorithms are slow and depend on heavy computation. One alternative found in the foruns were the XGBoost as an tree-based extreme gradient boosting algorithm.
